...
emit("变更块 =", JSON.stringify(found.map((block) => [block.id, block.start, block.end])));
emit("变更块数 =", found.length);
emit("应用的块 =", JSON.stringify(result.applied));
emit("冲突的块 =", JSON.stringify(result.conflicts));
emit("应用后的文本 =", JSON.stringify(result.lines));
